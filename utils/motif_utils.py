...
def get_attention_sequences(attention_file, sequence_file):
    """
    Function:
        Extracts enhancer and non-enhancer sequences along with their respective attention scores.

    Parameters:
        attention_file (str): Path to the file containing saved attention scores (NumPy format).
        sequence_file (str): Path to the file containing original sequences and labels (tab-separated).

    Returns:
        tuple:
            - pos_sequences (np.ndarray): Array of enhancer sequences.
            - neg_sequences (np.ndarray): Array of non-enhancer sequences.
            - pos_attention_scores (np.ndarray): Attention scores for enhancer sequences.
            - neg_attention_scores (np.ndarray): Attention scores for non-enhancer sequences.
    """
    # Load sequence data and attention scores
    df = pd.read_csv(sequence_file, sep="\t", header=None, names=["text", "label"])
    attention_scores = np.load(attention_file)
    print(f"Size: {attention_scores.shape}")

    # Extract sequences and labels
    sequences = df["text"].values
    labels = df["label"].values

    # Identify positive (enhancer) and negative (non-enhancer) indices
    neg_indices = np.where(labels == 0)[0]
    pos_indices = np.where(labels == 1)[0]

    # Split sequences and attention scores based on labels
    neg_sequences = sequences[neg_indices]
    pos_sequences = sequences[pos_indices]
    neg_attention_scores = attention_scores[neg_indices]
    pos_attention_scores = attention_scores[pos_indices]

    return pos_sequences, neg_sequences, pos_attention_scores, neg_attention_scores
...
def motif_analysis(pos_seqs,
    ...
    **kwargs -- other input arguments, may include:
    ...
    print(f"Before filtering, the length of motif: {len(motif_seqs)}")
    ...
    print(f"After filtering, the length of motif: {len(motif_seqs)}")
    ...
    print(f"After merger, the number of groups: {len(merged_motif_seqs)}")
    ...
    print(f"After number filter, the number of group: {len(merged_motif_seqs)}")
    ...
    if save_file_dir is not None:
        ...
        for motif, instances in merged_motif_seqs.items():
            ...
            m.weblogo(save_file_dir + "/motif_{}_{}_weblogo.png".format(len(instances["seq_idx"]), motif), format="png_print", show_fineprint=False, show_ends=False, color_scheme="color_classic")
